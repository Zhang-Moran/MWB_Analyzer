...
class FocalLoss(torch.nn.Module):
    def __init__(self, alpha=1, gamma=2, reduction='mean'):
        """
        :param alpha: Tensor of shape [n_classes] or a scalar. Weighting factor for each class.
        :param gamma: Focusing parameter (reduces loss for well-classified examples)
    ...
    def forward(self, inputs, targets):
        ...
        # Ensure alpha is correctly indexed
...
def train(opt, device):
    ...
    data_dir = r"/home/lsh/Public_Behavioral_Datasets/"
    # Dataloaders
    nc = 2

    train_images_path, train_images_label = read_video_data(os.path.join(data_dir, "Full_imgs"))

    dataset = VideoDataSet(video_path=train_images_path,
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(dataset, dataset.video_class)):
        ...
        LOGGER.info(f'Image sizes {imgsz} train, {imgsz} test\n'
                  ...
                  f'Starting {opt.model} training with {nc} classes for {epochs} epochs...\n\n'
        ...
        trainloader = torch.utils.data.DataLoader(train_subset,
                                                  ...
                                                  pin_memory=True,
                                                  num_workers=2)
                                                  #collate_fn=train_subset.collate_fn)

        testloader = torch.utils.data.DataLoader(val_subset,
                                             batch_size=bs // WORLD_SIZE,
                                             shuffle=False,
                                             pin_memory=True,
                                             num_workers=0)
                                             #collate_fn=val_subset.collate_fn)
        ...
        top1, top5, vloss, val_metrics = validate.run(model=ema.ema,
                                                    ...
                                                    criterion=criterion,
                                                    pbar=pbar,
                                                    save = True)  # test accuracy, loss

        fold_results.append(top1)
        print(f"Fold {fold + 1} Accuracy: {top1:.4f}")
